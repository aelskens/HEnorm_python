...
def rgb2he(rgb: np.array, stain: str, Io: int = 240, alpha: int = 1, beta: float = 0.15):
    """ Normalize staining appearence of H&E stained images
    
    Parameters
    ----------
    rgb : ndarray
        3D image to be transformed, np.array<height, width, ch>
    stain : str
    ...
    # calculate optical density
    OD = -np.log((rgb.astype(np.float) + 1) / Io)
    ...
    hem = np.multiply(Io, C2[0, :])
    eos = np.multiply(Io, C2[1, :])
    out = np.vstack((hem, eos))
    
    np.clip(out, a_min=0, a_max=254, out=out)
    out = np.reshape(out, (h, w, 2)).astype(np.uint8)
...
if __name__=='__main__':
    ...
    rgb = np.array(Image.open(args.imageFile))

    normalizeStaining(rgb = rgb,
